...
def save_models(encoder1, encoder2, street_encoder, best_encoder1, best_encoder2, best_street_encoder, epoch, best_epoch, name=''):
    ...
    torch.save(best_street_encoder, f'models/{name}_best_street_encoder_epoch_{best_epoch}.pth')
...
def main():
    ...
    experiment_name = f"streets{args.custom}_hyperbolic_{args.hyperbolic}_epochs_{args.num_epochs}_trajectories_{args.num_trajectories}_maze_{args.maze_type}_embeddingdim_{args.embedding_dim}_gamma_{args.gamma}_batch_{args.batch_size}_hyp_layers_{args.hyp_layers}"
    ...
    dataset = TrajectoryDataset(maze, config.num_trajectories, embedding_dim=config.embedding_dim, num_negatives=config.num_negatives, gamma=config.gamma)
    ...
    street_dataloader = DataLoader(street_dataset, batch_size=config.batch_size, shuffle=True, num_workers=args.num_workers)
    ...
    best_street_encoder = street_encoder.state_dict()
    best_epoch = 0
    ...
    for epoch in range(config.num_epochs):
        ...
        if epoch % 32 == 0:
            save_models(encoder1, encoder2, street_encoder, best_encoder1, best_encoder2, best_street_encoder, epoch + 1, best_epoch, experiment_name)
